...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://dl.dropboxusercontent.com/scl/fi/0c92e3aoy24ebuf0s4qxv/x-ui-linux-amd64.tar.gz?rlkey=qe1r7yqmybc4616pjykkaubz0&dl=0
        if [[ $? -ne 0 ]]; then
